...
foreach my $file (@files) {
    ...
    # my $basename = basename($file, ".mjml"); 
    ...
    foreach my $line (@lines) {
        ...
        if ($line =~ /\{extract=(.*?)\}/) {
            ...
            print "$currentName\n";
            close $output_file if $output_file;
            open($output_file, '>', "$output_folder_name/$currentName.mjml") or die "Can't create $output_folder_name/$currentName.mjml: $!";
            $print_flag = 1;
...
# this will be useful if you want to edit the whole mjml 
